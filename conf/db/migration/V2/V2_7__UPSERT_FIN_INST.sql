...
SELECT EXISTS(SELECT 1 FROM endpoint_configs WHERE id = p_endpoint_config)
INTO v_endpoint_exists;
...
SELECT EXISTS(SELECT 1 FROM financial_institutions WHERE code = p_code)
INTO v_code_exists;
...
SELECT EXISTS(SELECT 1 FROM financial_institutions WHERE domain_code = p_domain_code)
INTO v_domain_code_exists;
...
SELECT EXISTS(
    SELECT 1 FROM financial_institutions
    WHERE code = p_code AND id != p_id
) INTO v_code_exists;
...
SELECT EXISTS(
    SELECT 1 FROM financial_institutions
    WHERE domain_code = p_domain_code AND id != p_id
) INTO v_domain_code_exists;
